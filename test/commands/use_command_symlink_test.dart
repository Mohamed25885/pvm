...
void main() {
  group('UseCommand - Symlink Behavior Tests', () {
    ...
    test(
      'a) pvm use with no args and no .pvmrc file: exit code 1, no .pvm symlink created',
      () async {
    ...
    test(
      ...
      () async {
        ...
        final phpVer = FakePhpVersionManager(console);
        phpVer.readResult = null; // No .pvmrc file
    ...
    test(
      'c) pvm use with .pvmrc file present: reads version from file and creates correct symlink',
      () async {
        final osManager = MockOSManager();
        osManager.mockVersions = ['8.0', '8.2'];
        osManager.mockProgramDir = r'C:\pvm';
        ...
        final phpVer = FakePhpVersionManager(console);
        phpVer.readResult = PhpVersion.parse('8.0'); // .pvmrc contains 8.0
        ...
        // .pvmrc should NOT be rewritten (it already had the correct version)
        // Actually, according to UseCommand, when using .pvmrc, updateFile=true
    ...
    test(
      'e) pvm use with .pvmrc but version not installed: prompts pick and creates symlink',
      () async {
        final osManager = MockOSManager();
        osManager.mockVersions = ['8.0', '8.2'];
        osManager.mockProgramDir = r'C:\pvm';
        ...
        phpVer.readResult = PhpVersion.parse(
          '9.0',
        ); // .pvmrc has version not in available list
        phpVer.promptVersionPickResult = PhpVersion.parse(
          '8.2',
        ); // User picks 8.2
        ...
        // .pvmrc should be updated to picked version
        expect(phpVer.writeVersion, equals('8.2'));
      },
    );
  });
