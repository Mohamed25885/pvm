...
void main() {
  group('UseCommand - no-arg behavior', () {
    test('returns error when no version and no .pvmrc exists', () async {
    ...
    test('uses version from .pvmrc when no arg given', () async {
      final osManager = MockOSManager();
  ...
  group('UseCommand - version argument', () {
    test(
      'major.minor shorthand resolves when exactly one patch installed',
      () async {
        final osManager = MockOSManager();
        ...
        expect(exitCode, equals(0));
        expect(phpVer.writeVersion, equals('8.4.1'));
        expect(versionActivator.activateLocalVersion, equals('8.4.1'));
      },
    );
    ...
    test(
      ...
      () async {
        final osManager = MockOSManager();
        osManager.mockVersions = ['8.4.0', '8.4.1'];
        osManager.mockLocalPath = r'C:\project\.pvm';
        final console = MockConsole();
    ...
    test('valid version creates symlink and updates .pvmrc', () async {
      final osManager = MockOSManager();
      osManager.mockVersions = ['8.0', '8.2'];
      osManager.mockLocalPath = r'C:\project\.pvm';
      osManager.symlinkSourceExistsOverride = true;
      ...
      final phpVer = FakePhpVersionManager(console);
      phpVer.readResult = null; // no .pvmrc
  ...
  group('UseCommand - mismatch behavior (non-interactive)', () {
    test(
      'mismatch with non-interactive auto-applies without updating .pvmrc',
      () async {
        final osManager = MockOSManager();
        osManager.mockVersions = ['8.0', '8.2'];
        osManager.mockLocalPath = r'C:\project\.pvm';
        ...
        final phpVer = FakePhpVersionManager(console);
        phpVer.readResult = PhpVersion.parse('8.0'); // .pvmrc has 8.0
        ...
        expect(exitCode, equals(0));
        // Version is applied in non-interactive mode (activator called), .pvmrc unchanged
  ...
  group('UseCommand - GitIgnoreService auto-run', () {
    test(
      'runs GitIgnoreService on every use (but NOT early symlink creation)',
      () async {
        final osManager = MockOSManager();
  ...
  group('UseCommand - rootPath discovery', () {
    test('uses CWD when no .pvmrc is found up the tree', () async {
      final osManager = MockOSManager();
      osManager.mockVersions = ['8.0'];
      osManager.mockLocalPath = r'C:\project\.pvm';
      osManager.symlinkSourceExistsOverride = true;
      ...
      // Should still call GitIgnoreService (even if .pvmrc not found)
      expect(gitIgnore.ensureGitignoreCalled, isTrue);
    });
  });
}
